...
exports.system.publish = function( fromUserId, toUserIDs, objectName, content, pushContent, pushData, isPersisted, isCounted, format, callback ) {
  var requestParams = {
    fromUserId  : fromUserId,
    objectName  : objectName,
    content     : typeof content === 'object' ? JSON.stringify(content) : content,
  ...
  var optionalArgs = { '4' : 'pushContent', '5' : 'pushData', '6' : 'isPersisted', '7' : 'isCounted' };
  for( var i=4; i<=indexOfLastParam; ++i ) {
    if( optionalArgs[i] ) {
      requestParams[ optionalArgs[i] ] = arguments[i];
    }
...
exports.system.publish_template = function( fromUserId, toUserIDs, objectName, content, values, pushContent, pushData, isPersisted, isCounted, format, callback ) {
    var requestParams = {
        fromUserId  : fromUserId,
        objectName  : objectName,
        content     : typeof content === 'object' ? JSON.stringify(content) : content,
    ...
    rongrequest.requestWithHeaders( apis[ 'message']['system']['publish_template'], requestParams, format, { 'Content-Type' : 'application/json' }, function( err, resultText ) {
        return callback( err, resultText );
    } );
}
